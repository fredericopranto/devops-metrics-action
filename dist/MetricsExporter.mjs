...
export class MetricsExporter {
    static exportToCSV(results, nullResults, outputDir) {
        const csvContent = 'Repository,Category,Deployment Frequency (DF),DF Level,Lead Time (days),LT Level,Change Failure Rate (CFR),CFR Level,Mean Time to Restore (MTTR),MTTR Level\n' +
            results
    ...
    static exportToConsole(results, nullResults) {
        console.log('\n=== Metrics Report ===');
        ...
        if (results.length === 0) {
            ...
            results.forEach(r => {
                console.log(`${r.repository}, ${r.category}, ${r.dfValue}, ${r.dfLevel}, ${r.ltValue}, ${r.ltLevel}, ${r.cfrValue}, ${r.cfrLevel}, ${r.mttrValue}, ${r.mttrLevel}`);
            });
        }
...
//# sourceMappingURL=MetricsExporter.mjs.map